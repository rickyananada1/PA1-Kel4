        ...
        @foreach($peralatantanamen as $item)
          ...
          <br><br>
            <div class="slider-image text-center">
               <img src="{{  asset('images/alattanaman/'.$item->image) }}" class="d-inline-block border text-center rounded" alt="...">
            </div>
          </div>
         @endforeach
      </div>
...
<div class="container">
<h2>Peralatan Tanaman</h2>
<div class="row row-cols-1 row-cols-md-4 g-4 mt-2 shadow">
  @foreach($peralatantanamen as $item)
  <div class="col">
    <div class="card h-100">
      <img src=" {{ asset('images/alattanaman/'.$item->image) }}" width="100%" height="150px" alt="...">
      <div class="card-body">
        <h5 class="card-title">{{$item->name}}</h5>
        <p class="card-text">{{$item->description}}</p>
        <a href="{{ route('client.alattanaman.show', $item->id) }}" class="btn btn-primary">View Detail</a>
      </div>
    </div>
  </div>
@endforeach
